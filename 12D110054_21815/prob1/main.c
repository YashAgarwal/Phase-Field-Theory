...
sprintf(name,"./output/phi_%d.dat",0);
fpw=fopen(name,"w");
for(i1=0; i1<n_x; ++i1)
{
fprintf(fpw,"%le\n",__real__ comp[i1]);
...
for(INDEX=1; INDEX<=T; ++INDEX){
	...
	//initialize h
	for(i1=0; i1 < n_x; ++i1){
		h[i1] = 2*A*comp[i1]*(1-comp[i1])*(1-2*comp[i1]);
	}
	...
	for(i1=0; i1 < n_x; ++i1){
		...
		kx2 = kx*kx;
		comp[i1] = (comp[i1]-h[i1]*delta_t)/(1+alpha*kx2*delta_t);
	}
	...
	if(INDEX%T_write==0)
	{
		sprintf(name,"./output/phi_%d.dat",INDEX);
		fpw=fopen(name,"w");
		for(i1=0; i1<n_x; ++i1)
		{
			fprintf(fpw,"%le\n",__real__ comp[i1]);
